-- COMPLETE RLS FIX for FilmDate (v2)
...
CREATE POLICY "couples_select" ON public.couples FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "couples_insert" ON public.couples FOR INSERT WITH CHECK (auth.uid() = user1_id);
CREATE POLICY "couples_update" ON public.couples FOR UPDATE USING (true);
CREATE POLICY "couples_delete" ON public.couples FOR DELETE USING (auth.uid() = user1_id AND user2_id IS NULL);
...
CREATE POLICY "users_select" ON public.users FOR SELECT USING (
  auth.uid() = id
  OR EXISTS (SELECT 1 FROM public.couples c WHERE (c.user1_id = auth.uid() AND c.user2_id = id) OR (c.user2_id = auth.uid() AND c.user1_id = id))
);
...
CREATE POLICY "responses_select" ON public.challenge_responses FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "responses_insert" ON public.challenge_responses FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "responses_update" ON public.challenge_responses FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "collages_select" ON public.collages FOR SELECT USING (EXISTS (SELECT 1 FROM public.couples c WHERE c.id = couple_id AND (c.user1_id = auth.uid() OR c.user2_id = auth.uid())));
CREATE POLICY "collages_insert" ON public.collages FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.couples c WHERE c.id = couple_id AND (c.user1_id = auth.uid() OR c.user2_id = auth.uid())));
CREATE POLICY "collages_update" ON public.collages FOR UPDATE USING (EXISTS (SELECT 1 FROM public.couples c WHERE c.id = couple_id AND (c.user1_id = auth.uid() OR c.user2_id = auth.uid())));
CREATE POLICY "collages_delete" ON public.collages FOR DELETE USING (EXISTS (SELECT 1 FROM public.couples c WHERE c.id = couple_id AND (c.user1_id = auth.uid() OR c.user2_id = auth.uid())));
